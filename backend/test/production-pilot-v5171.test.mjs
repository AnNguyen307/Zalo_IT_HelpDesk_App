...
test("v5.17.2 Mini App deploy gate accepts the current ready v5.18.6 Backend", async () => {
  const [backendManifestText, miniAppManifestText, workflow, releaseNote, checklist, botRelease, webhookRelease, mobileRelease, accountMenuRelease, identityRelease] = await Promise.all([
    ...
    readFile(path.join(repositoryRoot, "docs", "releases", "v5.18.6", "CHANGES_V5_18_6_OFFICIAL_APP_IDENTITY.md"), "utf8"),
  ]);
  const backendManifest = JSON.parse(backendManifestText);
  const miniAppManifest = JSON.parse(miniAppManifestText);

  assert.equal(backendManifest.version, "5.18.6");
  assert.equal(miniAppManifest.version, "5.17.2");
  ...
  assert.match(accountMenuRelease, /Mini App build\/deployment: not required/);
  assert.match(identityRelease, /Mini App Testing deployment: required/);
});
