  ...
  $("#ticketDetail").innerHTML = `<div class="ticket-workbench">
    <div class="workbench-main">
      <article class="detail-card admin-conversation-card">
        ...
        <div class="admin-reply-dock"><div id="adminReplyFileList" class="admin-reply-file-list"></div><div class="admin-reply-composer"><label class="admin-reply-attach" title="Thêm ảnh hoặc file">▧<input id="adminReplyFilesInput" type="file" multiple accept="image/*,.pdf,.txt,.csv,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip"/></label><textarea id="adminReply" rows="2" placeholder="Nhập phản hồi cho người dùng…"></textarea><button id="sendReply" class="button primary-button" type="button">Gửi phản hồi</button></div><small class="reply-security-note">Tối đa 4 file · 30 MB/file · tổng 120 MB</small></div>
      </article>
    </div>
    <aside class="workbench-side">
      ...
      <article class="detail-card dispatch-card"><div class="card-head"><h3>Điều phối xử lý</h3><span class="badge">${state.user?.role === "admin" ? "ADMIN" : "TECHNICIAN"}</span></div><div class="control-grid"><div class="two"><label>Trạng thái<select id="editStatus">${["open", "waiting_user", "in_progress", "resolved", "closed"].map((value) => `<option value="${value}" ${ticket.status === value ? "selected" : ""}>${labels[value]}</option>`).join("")}</select></label><label>Ưu tiên<select id="editPriority">${["low", "normal", "high", "urgent"].map((value) => `<option value="${value}" ${ticket.priority === value ? "selected" : ""}>${labels[value]}</option>`).join("")}</select></label></div><label>Người phụ trách<input id="editAssignee" value="${esc(ticket.assignedTo || "")}" placeholder="Tên kỹ thuật viên" /></label><label>Ghi chú / giải pháp<textarea id="editResolution" rows="3" placeholder="Ghi nguyên nhân và cách xử lý…">${esc(ticket.resolution || "")}</textarea></label><button id="saveTicket" class="button primary-button save-button" type="button">Lưu cập nhật</button><button id="draftFromTicketBtn" class="button source-ticket-button" type="button">◫ Tạo đề xuất Playbook</button></div></article>

      <details class="detail-card admin-collapsible"><summary><span>Thông tin & SLA</span><b>＋</b></summary><div class="admin-collapsible-content"><div class="sla-panel"><div class="sla-box ${ticket.sla?.firstResponseOverdue ? "overdue" : ""}"><span>Phản hồi đầu tiên</span><strong>${ticket.sla?.firstRespondedAt ? "Đã phản hồi" : timeLeft(ticket.sla?.firstResponseDueAt)}</strong></div><div class="sla-box ${ticket.sla?.resolutionOverdue || ticket.sla?.overdue ? "overdue" : ""}"><span>Hạn hoàn tất</span><strong>${["resolved", "closed"].includes(ticket.status) ? "Đã hoàn tất" : timeLeft(ticket.sla?.resolutionDueAt)}</strong></div></div><div class="meta-grid"><div class="meta-box"><span>Danh mục</span><strong>${esc(labels[ticket.category] || ticket.category)}</strong></div><div class="meta-box"><span>Ưu tiên</span><strong>${esc(labels[ticket.priority] || ticket.priority)}</strong></div><div class="meta-box"><span>Phụ trách</span><strong>${esc(ticket.assignedTo || "Chưa phân công")}</strong></div><div class="meta-box"><span>Thiết bị</span><strong>${esc(ticket.device || "—")}</strong></div><div class="meta-box"><span>Vị trí</span><strong>${esc(ticket.location || "—")}</strong></div><div class="meta-box"><span>Tạo lúc</span><strong>${formatDate(ticket.createdAt)}</strong></div></div><div class="side-description"><strong>Mô tả ban đầu</strong><div class="description">${esc(ticket.description)}</div></div></div></details>

      <details class="detail-card admin-collapsible decision-collapsible"><summary><span>Quyết định AI</span><em class="badge ${guided ? "guide" : "escalate"}">${guided ? "GUIDE" : "ESCALATE"}</em></summary><div class="admin-collapsible-content"><div class="decision-panel ${guided ? "" : "escalated"}"><div class="decision-head"><span class="decision-symbol">${guided ? "✓" : "↗"}</span><div><strong>${guided ? "Hướng dẫn theo Playbook" : "Đã chuyển kỹ thuật viên"}</strong><small>${guided ? "Đủ điều kiện an toàn" : esc(escalationLabels[analysis.escalationCode] || "Không đủ điều kiện tự động xử lý")}</small></div></div><div class="decision-copy"><strong>${esc(analysis.summary || "Chưa có đánh giá.")}</strong>\n\n${esc(analysis.reason || "")}</div><div class="decision-meta"><span class="badge">${esc(analysis.source || "—")}</span><span class="badge">${esc(analysis.model || "Rules")}</span><span class="badge">${Math.round((analysis.confidence || 0) * 100)}%</span></div>${sourceHtml}</div></div></details>

      <details class="detail-card admin-collapsible" ${attachments.length ? "open" : ""}><summary><span>File đính kèm</span><em class="badge">${attachments.length}</em></summary><div class="admin-collapsible-content"><div class="attachment-admin-list">${attachmentHtml}</div><label class="admin-upload">＋ Tải thêm file<input id="adminFiles" type="file" multiple accept="image/*,.pdf,.txt,.csv,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip" /></label></div></details>
      <details class="detail-card admin-collapsible"><summary><span>Đánh giá hài lòng</span><b>＋</b></summary><div class="admin-collapsible-content"><div class="rating-admin">${ticket.satisfaction ? `<strong>${"★".repeat(ticket.satisfaction.score)}${"☆".repeat(5 - ticket.satisfaction.score)}</strong><p>${esc(ticket.satisfaction.comment || "Không có nhận xét")}</p><small>${formatDate(ticket.satisfaction.ratedAt)}</small>` : '<span class="muted">Người dùng chưa đánh giá.</span>'}</div></div></details>
      <details class="detail-card admin-collapsible"><summary><span>Lịch sử xử lý</span><em class="badge">${history.length}</em></summary><div class="admin-collapsible-content"><div class="history-admin">${historyHtml}</div></div></details>
    </aside>
  </div>`;
